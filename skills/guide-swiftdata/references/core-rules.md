...
- Do not attempt to use `@Query` outside of SwiftUI views; it is designed to work specifically *inside* views, and will not operate correctly outside. For more help with SwiftUI, use Apple's `swiftui-specialist` skill (Xcode 27) or the `swiftui` reference.
